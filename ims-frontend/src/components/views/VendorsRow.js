...
class VendorsRow extends React.Component{
    ...
    deleteVendors(){
        axio.delete('http://localhost:5000/api/vendor/' + this.props.obj.id)
            .then(response => {
                console.log("Vendors Deleted" + response.data);
            })
            .catch(error => {
    ...
    render() {
        return(
            <tr className="positive">
                ...
                <td>{this.props.obj.vendor_name}</td>
                <td>{this.props.obj.registered_date}</td>
                <td>
